...
contract MasterchefScalper is ERC20("SCALP-STABLE", "SCALP-STABLE"), Ownable, Pausable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount,address _referrar) public whenNotPaused nonReentrant {
        ...
        _earn();
        _stabiliseOf();
        uint256 value = pool.add(stableAssetBalance());
        if(totalshares == 0){
    ...
    function withdraw(uint256 _amount) public nonReentrant {
        ...
        uint256 stablepart = calc.sub(balanceOf(msg.sender));
        uint256 usershare = stablepart.mul(99).div(100);
        uint256 referrarshare = stablepart.div(100);
        uint256 tempshare = totalshares-share[msg.sender];
        uint256 tempsupply = baseAssetBalanceOf().add(stableAssetBalance()).sub(balanceOf(msg.sender)).sub(stablepart);
        if(tempshare==0){
...
//UI
    function getPricePerFullShare() public view returns (uint256) {
        return totalSupply() == 0 ? 1e18 : (baseAssetBalanceOf()+stableAssetBalance()).mul(1e18).div(totalshares);
    }
...
//Internal executions
    ...
    function _stabiliseOf() internal {
        ...
        uint256 currentPerformanceFee = findswapvalue.mul(performanceFee).div(10000);
        IERC20(stableAsset).safeTransfer(treasury, currentPerformanceFee);
        uint256 currentGovernanceShare = findswapvalue.mul(governanceDivident).div(10000);
...
*/
    ...
    function setUniRouter(address _unirouter) external onlyOwner {
        ...
        unirouter = _unirouter;
        IERC20(rewardAsset).safeApprove(unirouter, type(uint256).max);
    }
